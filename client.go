...
func (client *client) tryRefreshMetadata(topics []string, attemptsRemaining int, deadline time.Time) error {
	pastDeadline := func(backoff time.Duration) bool {
		if !deadline.IsZero() && time.Now().Add(backoff).After(deadline) {
			Logger.Println("[tryRefreshMetadata] client/metadata past the metadata timeout")
	...
	retry := func(err error) error {
		if attemptsRemaining > 0 {
			...
			if pastDeadline(backoff) {
				Logger.Println("[tryRefreshMetadata] client/metadata skipping last retries as we would go past the metadata timeout")
				return err
			}
	...
	for ; broker != nil && !pastDeadline(0); broker = client.any() {
		...
		if len(topics) > 0 {
			Logger.Printf("[tryRefreshMetadata] client/metadata fetching metadata for %v from broker(%d) %s\n", topics, broker.ID(), broker.addr)
		} else {
			allowAutoTopicCreation = false
			Logger.Printf("[tryRefreshMetadata] client/metadata fetching metadata for all topics from broker(%d) %s\n", broker.ID(), broker.addr)
		}
		...
		Logger.Printf("[tryRefreshMetadata] client/metadata got response from Kafka[broker(%d)], error = %v\n", broker.ID(), err)
		...
		case nil:
			...
			if shouldRetry {
				Logger.Printf("[tryRefreshMetadata] client/metadata found some partitions to be leaderless for broker(%d)\n", broker.ID())
		...
		case KError:
			...
			if err.(KError) == ErrSASLAuthenticationFailed {
				Logger.Printf("[tryRefreshMetadata] client/metadata failed SASL authentication for broker(%d)\n", broker.ID())
				return err
			}
			// else remove that broker and try again
			Logger.Printf("[tryRefreshMetadata] client/metadata got Kerror from broker(%d) while fetching metadata: %v\n", broker.ID(), err)
			_ = broker.Close()
			client.deregisterBroker(broker)
	...
	if broker != nil {
		Logger.Println("[tryRefreshMetadata] client/metadata not fetching metadata from broker(%d) %s as we would go past the metadata timeout\n", broker.ID(), broker.addr)
